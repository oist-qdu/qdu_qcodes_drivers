...
# Mikhail Belianchikov <experimenter021@gmail.com>, february/april 2025
import logging
import numpy as np
import cmath, math
...
from qcodes.utils.validators import Arrays,Numbers, Enum, Ints, Bool
...
class PointMagPhase(MultiParameter):
    ...
    def get_raw(self) -> Tuple[ParamRawDataType, ParamRawDataType]:
        ...
        if self.instrument.point_check_sweep_first:
            ...
            if self.instrument.stop() != self.instrument.start():
                raise ValueError('Stop-start should be equal. Please adjust'
                                'start or stop.')
