"""
This is a qcodes driver for LakeShore 270 temperature controller.
Derived from qcodes LakeShore_model_272 and lakeshore_base driver.
written by SERGEI.LEMZIAKOV@oist.jp
"""
...
# There are 16 sensors channels (a.k.a. measurement inputs) in Model 370
_n_channels = 16
...
class LakeshoreModel370Output(InstrumentChannel):
    ...
    def __init__(
        ...
        **kwargs: "Unpack[InstrumentBaseKWArgs]",
        ...
        """
        self.filter: GroupParameter = self.add_parameter(
            "filter",
        ...
        self.current_power: GroupParameter = self.add_parameter(
            "current_power",
            label="Current or power",
            docstring="Specifies heater output display in current or power",
            val_mapping={"current": 1, "power": 2},
            parameter_class=GroupParameter,
        )
        self.htr_limit: GroupParameter = self.add_parameter(
            "htr_limit",
            label="Heater range",
            docstring="Maximum heater range",
            val_mapping=self.RANGES,
            parameter_class=GroupParameter,
        )
        self.heater_resistance: GroupParameter = self.add_parameter(
            "heater_resistance",
            label="Heater resistance",
            unit="Ohm",
            docstring="Heater load in ohms",
            vals=vals.Numbers(1, 100000),
            get_parser=float,
            parameter_class=GroupParameter,
        )
        self.control_setup = Group(
            [self.input_channel, 
             self.filter, 
             self.units,
             self.delay,
        ...
        self.setpoint_ramp_group = Group(
            ...
            set_cmd=f"RAMP {{setpoint_ramping_enabled}},{{setpoint_ramping_rate}}",
            get_cmd=f"RAMP?",
        )
...
class LakeshoreModel370Channel(InstrumentChannel):
    """
    ...
    def __init__(
        ...
        **kwargs: "Unpack[InstrumentBaseKWArgs]",
        ...
        self.output_group = Group(
            ...
            set_cmd=f"RDGRNG {self._channel}, "
            f"{{excitation_mode}}, "
            f"{{excitation_range_number}}, "
            f"{{range}}, {{auto_range}}, "
            f"{{current_source_shunted}}, ",
            get_cmd=f"RDGRNG? {self._channel}",
        )
...
class LakeshoreModel370(VisaInstrument):
    """
    ...
    def __init__(
        ...
        **kwargs: "Unpack[VisaInstrumentKWArgs]",
        ...
        self.visa_handle.stop_bits = 10
        self.visa_handle.parity = 1
        ...
        heaters = {"sample_heater": 0}
        for heater_name, heater_index in heaters.items():
            self.add_submodule(
                heater_name, LakeshoreModel370Output(self, heater_name, heater_index)
            )
        for submod_name, submod in self.submodules.items():
            for param_name, parameter in submod.parameters.items():
                setattr(parameter, 'post_delay', 0.11)
